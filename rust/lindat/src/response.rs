...
fn add_file(path : &str, head : bool) -> Result<Response, Box<dyn Error>> {
    let mut root = String::from("./htdocs");

    if env::var("LINDAT_ROOT").is_ok() {
        root = env::var("LINDAT_ROOT").unwrap();
    };
        ...
        Err(e) => {
            response.status = match e.kind() {
                ErrorKind::NotFound => {
                    ...
                    if !head {
                        response.body = Some(fs::read(format!("{}404.html", root)).unwrap_or_else(|_| vec![]));
...
pub fn response(request : &Request) -> Result<Response, Box<dyn Error>> {
    match *request.method() {
        Method::GET => add_file(request.uri().to_str().expect("Invalid file URI utf8"), false),
